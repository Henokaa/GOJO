...
function addRenter(){
            ...
            if(!isset($_COOKIE["Gojo"])){setcookie("Gojo",$_POST["email"],time()+60*60*24*30);}
            ...
            if(isset($_SESSION["cookies"])&&$_SESSION["cookies"]=="true"){
                setcookie("Gojo",$_POST["email"],time()+60*60*24*30);
            }
...
function updateRenter($prevPassword,$previmage){
            ...
            $waterAndElectricity = $_POST['utility'];
            $transport = $_POST['transportation'];
            mysqli_query($connection,"UPDATE registrations SET renter_name='$name',renter_email='$email',renter_password='$password',renter_city='$city',renter_area='$area',renter_price=$price,renter_beds=$beds,renter_phone=$phone,renter_infrustructure='$waterAndElectricity',renter_house_picture='$image_name',renter_transportation='$transport' WHERE renter_email='{$_GET['email']}'");
...
function displayResult(){
    ...
    $max = $min+4;
    $count = 1;
    $rows_count = mysqli_num_rows($renters_array);
    if($rows_count==0){
        ?>
        <h1 style="text-align:center;font-size:2em">No Result Found</h1>
        <?php
    }
    while($row = mysqli_fetch_assoc($renters_array)){
        if($count>=$min&&$count<=$max){
            if(isset($_COOKIE['Gojo'])){
                setcookie("Gojo","$location",time()+60*60*24*30);
            }
            ?>
            <div class='card'>
